...
var Draggables = {
  ...
  _cacheObserverCallbacks: function () {
    ["onStart", "onEnd", "onDrag"].each(function (eventName) {
      Draggables[eventName + "Count"] = Draggables.observers.select(function (
        o
      ) {
        return o[eventName];
      }).length;
...
var Draggable = Class.create({
  initialize: function (element) {
    var defaults = {
      ...
      reverteffect: function (element, top_offset, left_offset) {
        var dur =
          Math.sqrt(Math.abs(top_offset ^ 2) + Math.abs(left_offset ^ 2)) *
          0.02;
  ...
  startScrolling: function (speed) {
    ...
    this.scrollSpeed = [
      speed[0] * this.options.scrollSpeed,
      ...
      with (this._getWindowScroll(this.options.scroll)) {
        if (this.scrollSpeed[0] || this.scrollSpeed[1]) {
          ...
          this.options.scroll.scrollTo(
            left + d * this.scrollSpeed[0],
            top + d * this.scrollSpeed[1]
          );
        }
      }
    } else {
...
var Sortable = {
  ...
  create: function (element) {
    ...
    (options.elements || this.findElements(element, options) || []).each(
      function (e, i) {
        var handle = options.handles
          ? $(options.handles[i])
          : options.handle
          ? $(e).select("." + options.handle)[0]
          : e;
        options.draggables.push(
          new Draggable(
            e,
            Object.extend(options_for_draggable, { handle: handle })
          )
        );
  ...
  onEmptyHover: function (element, dropon, overlap) {
    ...
    if (!Element.isParent(dropon, element)) {
      ...
      if (children) {
        var offset =
          Element.offsetSize(dropon, droponOptions.overlap) * (1.0 - overlap);
        ...
        for (index = 0; index < children.length; index += 1) {
          if (
            ...
            offset -= Element.offsetSize(
              children[index],
              droponOptions.overlap
            );
          } else if (
            offset -
              Element.offsetSize(children[index], droponOptions.overlap) / 2 >=
            0
          ) {
  ...
  serialize: function (element) {
    ...
    if (options.tree) {
      return Sortable.tree(element, arguments[1])
        .children.map(function (item) {
          return [
            name +
              Sortable._constructIndex(item) +
              "[id]=" +
      ...
      return Sortable.sequence(element, arguments[1])
        .map(function (item) {
...
Element.offsetSize = function (element, type) {
  return element[
    "offset" + (type == "vertical" || type == "height" ? "Height" : "Width")
  ];
};
