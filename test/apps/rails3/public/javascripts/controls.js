...
Autocompleter.Base = Class.create({
  ...
  updateElement: function (selectedElement) {
    ...
    if (bounds[0] != -1) {
      ...
      this.element.value =
        newValue + value + this.element.value.substr(bounds[1]);
    } else {
  ...
  getTokenBounds: function () {
    ...
    var diff = arguments.callee.getFirstDifferencePos(
      value,
    ...
    this.options = Object.extend(
      {
        ...
        selector: function (instance) {
          ...
          for (
            var i = 0;
            i < instance.options.array.length &&
            ret.length < instance.options.choices;
            i++
          ) {
          ...
          if (partial.length)
            ret = ret.concat(
              partial.slice(0, instance.options.choices - ret.length)
            );
...
Ajax.InPlaceEditor = Class.create({
  ...
  createControl: function (mode, handler, extraClasses) {
    ...
    if ("button" == control) {
      ...
      link.onclick =
        "cancel" == mode ? this._boundCancelHandler : this._boundSubmitHandler;
  ...
  createEditField: function () {
    var text = this.options.loadTextURL
    ...
    if (1 >= this.options.rows && !/\r|\n/.test(this.getText())) {
      ...
      fld.rows =
        1 >= this.options.rows ? this.options.autoRows : this.options.rows;
...
Ajax.InPlaceCollectionEditor = Class.create(Ajax.InPlaceEditor, {
  ...
  buildOptionList: function () {
    ...
    var textFound = this._collection.any(
      function (entry) {
