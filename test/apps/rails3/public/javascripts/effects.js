...
var Effect = {
  ...
  multiple: function (element, effect) {
    var elements;
    if (
      (typeof element == "object" || Object.isFunction(element)) &&
      element.length
    )
      elements = element;
    ...
    $A(elements).each(function (element, index) {
      new effect(
        element,
        Object.extend(options, { delay: index * options.speed + masterDelay })
      );
    });
  },
  PAIRS: {
    ...
    this.render = (function () {
      ...
      return function (pos) {
        ...
        if (this.state === "running") {
          pos =
            this.options.transition(pos) * this.fromToDelta + this.options.from;
          this.position = pos;
    ...
    this.element.setStyle({
      backgroundColor: $R(0, 2).inject(
        ...
        function (m, v, i) {
          return (
            m +
            (this._base[i] + this._delta[i] * position).round().toColorPart()
          );
        }.bind(this)
      ),
    });
  ...
  var options = arguments[1] || {},
    ...
    reverser = function (pos) {
      return (
        1 -
        transition(
          -Math.cos(pos * (options.pulses || 5) * 2 * Math.PI) / 2 + 0.5
        )
      );
    };
          ...
          ? "#" +
            Math.round(
              transform.originalValue[0] +
                (transform.targetValue[0] - transform.originalValue[0]) *
                  position
            ).toColorPart() +
            Math.round(
              transform.originalValue[1] +
                (transform.targetValue[1] - transform.originalValue[1]) *
                  position
            ).toColorPart() +
            Math.round(
              transform.originalValue[2] +
                (transform.targetValue[2] - transform.originalValue[2]) *
                  position
            ).toColorPart()
          : (
              transform.originalValue +
              (transform.targetValue - transform.originalValue) * position
            ).toFixed(3) + (transform.unit === null ? "" : transform.unit);
...
Effect.Methods = {
  morph: function (element, style) {
    ...
    new Effect.Morph(
      element,
      Object.extend({ style: style }, arguments[2] || {})
    );
    return element;
  },
